resource "aws_nat_gateway" "us_east_1a" {
  ...
  tags          = merge({ Name = "${var.vpc_resources.vpc}-${var.vpc_resources.nat_gateway_us_east_1a}" }, var.tags)
...
resource "aws_nat_gateway" "us_east_1b" {
  ...
  tags          = merge({ Name = "${var.vpc_resources.vpc}-${var.vpc_resources.nat_gateway_us_east_1b}" }, var.tags)
